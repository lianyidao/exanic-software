...
int main(int argc, char *argv[])
{
    ...
    int matched_chunks = 0, error_frames = 0;
    int verbose = 0;
    int more;
    ...
    {
        cur_time = time(NULL);
        size = exanic_receive_chunk_ex(rx, buf, &more, &info);
        //size = exanic_receive_frame(rx, buf, sizeof(buf), NULL);
        if (size > 0)
        {
            if (verbose)
            {
                printf("Chunk matches,size=%d\n", size);
                print_info(&info);
        ...
        else if (size < 0)
        {
            ...
            if (more) exanic_receive_abort(rx);
            error_frames++;
        }
